...
export function SampleCallout({
  ...
  return (
    <aside
      aria-label={label}
      className={clsx(
        "flex flex-col gap-6 rounded-md bg-[#222525]/4 p-8 sm:flex-row sm:items-stretch sm:gap-10 sm:p-10",
        className,
      )}
    >
      <div className="flex h-32 w-full flex-none items-center justify-center overflow-hidden rounded-sm bg-cream sm:h-auto sm:w-40">
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-3">
        ...
        <p className="font-heading text-2xl font-bold leading-snug tracking-tight text-brand-navy sm:text-3xl">
          {title}
        </p>
        {children && (
          <div className="text-base leading-relaxed text-brand-navy/70">
            {children}
          </div>
        )}
        {cta && (
